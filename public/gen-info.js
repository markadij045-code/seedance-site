(function(){
  ...
  ms.textContent=''
  +'.ws>.hwH1,.ws>.hwH1sub,.ws>.hwGuideLink{grid-column:1/-1;order:-2}'
  +'.ws>aside{order:-1}'
  +'.sc-ctrl{flex-wrap:wrap}'
  +'.sc-link{white-space:normal;text-align:center}'
  +'body{overflow-x:hidden}'
  +'@media(max-width:900px){'
  +'.nav-in{gap:10px;padding:10px 14px}'
  +'.nav-in a.btn-ghost{display:none}'
  +'.logo{font-size:1rem}'
  +'#hwFloat{top:70px;bottom:auto;right:12px;width:46px;height:46px;font-size:1.3rem;box-shadow:0 4px 14px rgba(163,230,53,.4)}'
  +'.hwGuideLink{margin:10px 0 16px}'
  +'.hwGuideLink a{font-size:.8rem;padding:9px 14px}'
  ...
  +'.bar-in{flex-direction:column;align-items:stretch;gap:7px;padding:9px 14px}'
  +'.bar-price{order:1;font-size:1.15rem}'
  +'.bar .btn{order:2;width:100%;margin-left:0;padding:12px 16px;font-size:.95rem}'
  +'.agree{order:3;min-width:0;font-size:.68rem}'
  +'#status{order:4}'
