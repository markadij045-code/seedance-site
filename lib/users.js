...
const USERS_PREFIX = () => 'users-' + secret();
const PRE_PREFIX = (email) => 'p-' + sha(secret() + ':' + email).slice(0, 24);
...
async function writeNew(prefix, data){
...
export async function readUsers(){
  const d = await readLatest(USERS_PREFIX(), { users: [], sessions: [] });
  if(!Array.isArray(d.users)) d.users = [];
  if(!Array.isArray(d.sessions)) d.sessions = [];
  return d;
}
export const writeUsers = (d) => writeNew(USERS_PREFIX(), d);
...
export async function listPresets(email){
  const d = await readLatest(PRE_PREFIX(email), { presets: [] });
...
export async function savePreset(email, preset){
  ...
  await writeNew(PRE_PREFIX(email), d);
  return d.presets;
}
